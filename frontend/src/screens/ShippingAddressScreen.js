...
export default function ShippingAddressScreen() {
  ...
  const [fullName_ship, setFullName_ship] = useState(
    shippingAddress.fullName_ship
  );
  const [address_ship, setAddress_ship] = useState(
    shippingAddress.address_ship
  );
  const [city_ship, setCity_ship] = useState(shippingAddress.city_ship);
  const [postalCode_ship, setPostalCode_ship] = useState(
    shippingAddress.postalCode_ship
  );
  const [country_ship, setCOuntry_ship] = useState(
    shippingAddress.country_ship
  );
  ...
  const submitHandler = (e) => {
    ...
    if (
      ...
      dispatch(
        saveBillingAddress({ fullName, address, city, postalCode, country })
      );
      dispatch(
        saveShippingAddress({
          fullName_ship,
