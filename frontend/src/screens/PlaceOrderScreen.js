...
export default function PlaceOrderScreen() {
    ...
    cartItems.map((item) =>
      reviewPromises.push(
        fetch(`${config.url.API_URL}/api/products/${item.product}`)
          ...
          .then((data) => {
            return { title: item.product, price: data.price * item.qty };
          })
      )
    );
  ...
  const totalfinal_priceHorizontal = () => {
    let num = 0;
    cartItems_hor.map((item) => (num = num + calculatefinal_priceHor(item)));
  ...
  const calculatefinal_priceHor = (item) => {
    ...
    let final_price =
      (price_width_heigth +
        Number(
          (width / 1000).toFixed(2) * (height / 1000).toFixed(2) * price_prof
        ) +
        Number(
          (width / 1000).toFixed(2) * (height / 1000).toFixed(2) * price_lam
        )) *
      amount;
  ...
  const getPriceProfile = (profile_color) => {
    if (
      profile_color == "zlatý dub" ||
      profile_color == "třešeň amaretto" ||
      profile_color == "tmavý dub" ||
      profile_color == "vlašský ořech" ||
      profile_color == "sapeli" ||
      profile_color == "přírodní dub" ||
      profile_color == "tmavý ořech" ||
      profile_color == "douglas" ||
      profile_color == "borovice horská"
    ) {
      return 215;
    } else return 0;
  };
  ...
  const placeOrderHandler = () => {
    dispatch(
      createOrder(cart, (final_price_naradni + final_priceHor).toFixed(2))
    );
  };
  ...
  const makePayment = (token) => {
    ...
    if (tot_price >= 100) {
      ...
      swal(
        "The checkout amount is not enough to conclude the purchase, minumum required is 100 kc"
      );
    }
  };
              ...
              <ul>
                ...
                <div className="ship_info">
                  ...
                  <p>
                    <span className="fullname">
                      <strong>Name: </strong>
                      {cart.shippingAddress.fullName_ship}
                    </span>
                    <br />
                    <span className="address">
                      <strong>Address: </strong>
                      {cart.shippingAddress.address_ship},&nbsp;
                      {cart.shippingAddress.city_ship},&nbsp;
                      {cart.shippingAddress.postalCode_ship},&nbsp;
                      {cart.shippingAddress.country_ship}
                    </span>
                  </p>
                </div>
                ...
                <div className="card card-body">
                  <h2>Order Items</h2>
                  {cartItems.map((item) => (
                    <CartItemOrder key={item.product} item={item} />
                  ))}
                </div>

                <div className="placeOrderScreen__left_bottom">
                  {cartItems_hor.map((item) => (
                    <CartItemOrderHor key={item.product} item={item} />
                  ))}
                </div>
              </ul>
